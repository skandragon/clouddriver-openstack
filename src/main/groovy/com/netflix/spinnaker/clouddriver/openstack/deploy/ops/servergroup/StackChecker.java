  ...
  public boolean isReady(Stack stack) {
    if (stack == null) {
      ...
      ActionResponse actionResponse =
          ActionResponse.actionFailed("Cannot get state for null stack", 404);
    ...
    if ((operationString + "_IN_PROGRESS").equals(status)) {
      ...
      String message =
          String.format(
              "Failed to create stack %s: %s", stack.getName(), stack.getStackStatusReason());
      ...
      String message =
          String.format(
              "Unknown status for stack %s: %s %s",
              stack.getName(), stack.getStatus(), stack.getStackStatusReason());
      ActionResponse actionResponse = ActionResponse.actionFailed(message, 500);
      throw new OpenstackProviderException(actionResponse);
    }
  }
