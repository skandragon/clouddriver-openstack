...
class UpsertOpenstackLoadBalancerAtomicOperation extends AbstractOpenstackLoadBalancerAtomicOperation implements AtomicOperation<Map>, TaskStatusAware {
  String PHASE = "UPSERT_LOADBALANCER_PHASE"
  ...
  Map operate(List priorOutputs) {
    task.updateStatus PHASE, "Initializing upsert of load balancer ${description.id ?: description.name} in ${description.region}..."
    ...
    try {
      ...
      if (listenersToDelete) {
        ...
        deleteLoadBalancerPeripherals(PHASE, region, resultLoadBalancer.id, deleteValues)
      }
      ...
      if (description.id && (!listenersToAdd.isEmpty() || !listenersToDelete.isEmpty())) {
        updateServerGroup(PHASE, region, resultLoadBalancer.id)
      }
    } catch (OpenstackProviderException ope) {
      throw new OpenstackOperationException(AtomicOperations.UPSERT_LOAD_BALANCER, ope)
    }

    task.updateStatus PHASE, "Done upserting load balancer ${resultLoadBalancer?.name} in ${region}"
  ...
  protected LoadBalancerV2 createLoadBalancer(String region, String name, String subnetId) {
    task.updateStatus PHASE, "Creating load balancer $name in ${region} ..."
    ...
    task.updateStatus PHASE, "Waiting on creation of load balancer $name in ${region} ..."
    ...
    task.updateStatus PHASE, "Created load balancer $name in ${region}."
    result
  }
  ...
  protected void updateSecurityGroups(String region, String portId, List<String> securityGroups) {
    ...
    if (securityGroups && port.securityGroups != securityGroups) {
      task.updateStatus PHASE, "Updating port ${portId} with security groups ${securityGroups} in ${region}..."
      provider.updatePort(region, portId, securityGroups)
      task.updateStatus PHASE, "Updated port ${portId} with security groups ${securityGroups} in ${region}."
    }
  }
  ...
  protected void updateFloatingIp(String region, String networkId, String portId) {
    ...
    if (networkId) {
      ...
      if (!existingFloatingIp) {
        task.updateStatus PHASE, "Associating floating IP ${ip.floatingIpAddress} with ${portId}..."
        provider.associateFloatingIpToPort(region, ip.id, portId)
        task.updateStatus PHASE, "Associated floating IP ${ip.floatingIpAddress} with ${portId}."
      } else {
        if (networkId != existingFloatingIp.floatingNetworkId) {
          task.updateStatus PHASE, "Disassociating ip ${existingFloatingIp.floatingIpAddress} and associating ip ${ip.floatingIpAddress} with vip ${portId}..."
          provider.disassociateFloatingIpFromPort(region, existingFloatingIp.id)
          provider.associateFloatingIpToPort(region, ip.id, portId)
          task.updateStatus PHASE, "Disassociated ip ${existingFloatingIp.id} and associated ip ${ip.floatingIpAddress} with vip ${portId}."
        }
      }
    } else {
      if (existingFloatingIp) {
        task.updateStatus PHASE, "Disassociating ip ${existingFloatingIp.floatingIpAddress} with vip ${portId}..."
        provider.disassociateFloatingIpFromPort(region, existingFloatingIp.id)
        task.updateStatus PHASE, "Disassociated ip ${existingFloatingIp.floatingIpAddress} with vip ${portId}."
      }
    }
  }
  ...
  protected void addListenersAndPools(String region, String loadBalancerId, String name, Algorithm algorithm, Map<String, Listener> listeners, HealthMonitor healthMonitor) {
    listeners?.each { String key, Listener currentListener ->
      task.updateStatus PHASE, "Creating listener $name in ${region}"
      ...
      task.updateStatus PHASE, "Waiting on creation of listener $name in ${region}"
      ...
      task.updateStatus PHASE, "Created listener $name in ${region}"

      task.updateStatus PHASE, "Creating pool $name in ${region}"
      ...
      task.updateStatus PHASE, "Waiting on creation of pool $name in ${region}"
      LoadBalancerChecker.from(openstackCredentials.credentials.lbaasConfig, LoadBalancerChecker.Operation.UPDATE).execute {
        provider.getLoadBalancer(region, loadBalancerId)
      }
      task.updateStatus PHASE, "Created pool $name in ${region}"
  ...
  protected void updateListenersAndPools(String region, String loadBalancerId, Algorithm algorithm, Collection<ListenerV2> listeners, HealthMonitor healthMonitor) {
    listeners?.each { ListenerV2 currentListener ->
      ...
      if (lbPool.lbMethod.name() != algorithm.name()) {
        task.updateStatus PHASE, "Updating pool $lbPool.name in ${region} ..."
        provider.updatePool(region, lbPool.id, algorithm.name())
        task.updateStatus PHASE, "waiting on update for pool $lbPool.name in ${region} ..."
        LoadBalancerChecker.from(openstackCredentials.credentials.lbaasConfig, LoadBalancerChecker.Operation.UPDATE).execute {
          provider.getLoadBalancer(region, loadBalancerId)
        }
        task.updateStatus PHASE, "Updated pool $lbPool.name in ${region}."
      }
    ...
    if (lbPool.healthMonitorId) {
      if (healthMonitor) {
        ...
        if (existingMonitor.type.name() == healthMonitor.type.name()) {
          task.updateStatus PHASE, "Updating health monitor $lbPool.name in ${region} ..."
          provider.updateMonitor(region, lbPool.healthMonitorId, healthMonitor)
          task.updateStatus PHASE, "Waiting on update to health monitor $lbPool.name in ${region} ..."
          LoadBalancerChecker.from(openstackCredentials.credentials.lbaasConfig, LoadBalancerChecker.Operation.UPDATE).execute {
            provider.getLoadBalancer(region, loadBalancerId)
          }
          task.updateStatus PHASE, "Updated health monitor $lbPool.name in ${region}."
        } else {
          task.updateStatus PHASE, "Removing existing monitor ${existingMonitor.id} and creating health monitor for ${lbPool.name} in ${region}..."
          provider.deleteMonitor(region, lbPool.healthMonitorId)
          task.updateStatus PHASE, "Waiting on remove of existing monitor ${existingMonitor.id} in ${region}"
          LoadBalancerChecker.from(openstackCredentials.credentials.lbaasConfig, LoadBalancerChecker.Operation.UPDATE).execute {
            provider.getLoadBalancer(region, loadBalancerId)
          }
          task.updateStatus PHASE, "Waiting on creattion of health monitor for ${lbPool.name} in ${region}..."
          provider.createMonitor(region, lbPool.id, healthMonitor)
          LoadBalancerChecker.from(openstackCredentials.credentials.lbaasConfig, LoadBalancerChecker.Operation.UPDATE).execute {
            provider.getLoadBalancer(region, loadBalancerId)
          }
          task.updateStatus PHASE, "Removed existing monitor ${existingMonitor.id} and created health monitor for ${lbPool.name} in ${region}."
        }
      } else {
        removeHealthMonitor(PHASE, region, loadBalancerId, lbPool.healthMonitorId)
      }
    } else {
      if (healthMonitor) {
        task.updateStatus PHASE, "Creating health monitor for pool $lbPool.name in ${region} ..."
        provider.createMonitor(region, lbPool.id, healthMonitor)
        task.updateStatus PHASE, "Waiting on creation of health monitor for pool $lbPool.name in ${region} ..."
        LoadBalancerChecker.from(openstackCredentials.credentials.lbaasConfig, LoadBalancerChecker.Operation.UPDATE).execute {
          provider.getLoadBalancer(region, loadBalancerId)
        }
        task.updateStatus PHASE, "Created health monitor for pool $lbPool.name in ${region}."
      }
    }
  }
}
