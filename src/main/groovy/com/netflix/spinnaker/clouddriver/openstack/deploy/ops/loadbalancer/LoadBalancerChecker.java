...
class LoadBalancerChecker implements BlockingStatusChecker.StatusChecker<LoadBalancerV2> {
  ...
  public boolean isReady(LoadBalancerV2 loadBalancer) {
    if (loadBalancer == null) {
      ...
      ActionResponse actionResponse =
          ActionResponse.actionFailed("Cannot get status for null loadbalancer", 404);
    ...
    if (status == LbProvisioningStatus.ERROR) {
      String failureMessage =
          String.format(
              "Error in load balancer provision: %s, %s",
              loadBalancer.getName(), loadBalancer.getId());
  ...
  static BlockingStatusChecker from(LbaasConfig lbaasConfig, Operation operation) {
    ...
    return BlockingStatusChecker.from(
        lbaasConfig.getPollTimeout(), lbaasConfig.getPollInterval(), checker);
  }
}
